...
function chooseSameDateSession(remote: Record<string, any>, local: Record<string, any>): "remote" | "local" {
...
function mergeSession(remoteValue: unknown, localValue: unknown) {
  ...
  const winner = chooseSameDateSession(remote, local);
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!readOnly) {
      const { error: writeError } = await supabase.from("daily_fifty_sync_state").upsert({
        id: PROFILE_ID,
        payload: merged,
        updated_at: now,
      });
